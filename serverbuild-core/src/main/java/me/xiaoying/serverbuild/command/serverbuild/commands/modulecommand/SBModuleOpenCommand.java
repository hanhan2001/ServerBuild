...
@Command(values = "open", length = 1, description = "开启 Module")
public class SBModuleOpenCommand extends SCommand {
    ...
    public void performCommand(CommandSender sender, String[] args) {
        ...
        if (module.isEnabled()) {
            sender.sendMessage(new VariableFactory(ConfigCommon.OVERALL_SITUATION_MESSAGE_MODULE_OPENED).prefix(ConfigCommon.OVERALL_SITUATION_VARIABLE_PREFIX).date(ConfigCommon.OVERALL_SITUATION_VARIABLE_DATEFORAMT).module(module).placeholder(sender).color().toString());
            return;
        }

        module.init();
        module.enable();
        sender.sendMessage(new VariableFactory(ConfigCommon.OVERALL_SITUATION_MESSAGE_MODULE_OPEN).prefix(ConfigCommon.OVERALL_SITUATION_VARIABLE_PREFIX).date(ConfigCommon.OVERALL_SITUATION_VARIABLE_DATEFORAMT).module(module).placeholder(sender).color().toString());
    }
