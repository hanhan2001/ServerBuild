...
public class SBModuleInfoCommand extends SCommand {
    ...
    public void performCommand(CommandSender sender, String[] args) {
        ...
        if ((module = SBPlugin.getModuleManager().getModule(args[0])) == null) {
            sender.sendMessage(new VariableFactory(ConfigCommon.OVERALL_SITUATION_MESSAGE_MODULE_NOT_FOUND).prefix(ConfigCommon.OVERALL_SITUATION_VARIABLE_PREFIX).date(ConfigCommon.OVERALL_SITUATION_VARIABLE_DATEFORAMT).placeholder(sender).color().toString());
            return;
        }

        sender.sendMessage(new VariableFactory(ConfigCommon.OVERALL_SITUATION_MESSAGE_MODULE_INFO)
                .prefix(ConfigCommon.OVERALL_SITUATION_VARIABLE_PREFIX)
                .date(ConfigCommon.OVERALL_SITUATION_VARIABLE_DATEFORAMT)
                .module(module)
                .module_exact(module)
