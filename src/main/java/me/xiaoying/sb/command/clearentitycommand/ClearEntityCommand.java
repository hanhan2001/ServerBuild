package me.xiaoying.sb.command.clearentitycommand;

import me.xiaoying.sb.command.RegisteredCommand;
import me.xiaoying.sb.command.SubCommand;
import me.xiaoying.sb.command.clearentitycommand.subcommand.CEReloadCommand;
import me.xiaoying.sb.command.clearentitycommand.subcommand.CEClearCommand;
...
public class ClearEntityCommand implements TabExecutor {
    private final Map<String, List<RegisteredCommand>> registeredCommands = new HashMap<>();

    public ClearEntityCommand() {
        this.registerCommand(new CEClearCommand());
        this.registerCommand(new CEReloadCommand());
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        if (args == null || args.length == 0) {
            ClearEntityConstant.MESSAGE_HELP.forEach(string -> sender.sendMessage(ColorUtil.translate(string)));
            return false;
        }
