...
public class AdminReloadCommand extends SubCommand {
    ...
    public boolean performCommand(CommandSender sender, String[] args) {
        if (!sender.hasPermission("sb.admin")) {
            sender.sendMessage(ColorUtil.translate(FileConfig.OVERALL_MESSAGE_PREFIX + FileConfig.OVERALL_MESSAGE_NOPERMISSION));
            return false;
        }

        Handler.reloadHandles();
        ServerBuild.clear();
        sender.sendMessage(ColorUtil.translate(FileConfig.OVERALL_MESSAGE_PREFIX + FileConfig.OVERALL_MESSAGE_RELOAD));
        return false;
    }
}