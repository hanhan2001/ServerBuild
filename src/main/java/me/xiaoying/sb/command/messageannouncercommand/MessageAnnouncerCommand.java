package me.xiaoying.sb.command.messageannouncercommand;

import me.xiaoying.sb.command.RegisteredCommand;
import me.xiaoying.sb.command.SubCommand;
import me.xiaoying.sb.command.messageannouncercommand.subcommand.MAReload;
 ...
 * 命令 MessageAnnouncer
 */
public class MessageAnnouncerCommand implements TabExecutor {
    private final Map<String, List<RegisteredCommand>> registeredCommands = new HashMap<>();

    public MessageAnnouncerCommand() {
        this.registerCommand(new MAReload());
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        if (args == null || args.length == 0) {
            MessageAnnouncerConstant.MESSAGE_HELP.forEach(string -> sender.sendMessage(ColorUtil.translate(string)));
            return false;
        }
